...
public class KalmanInfoHelper extends BaseDao{
	
	//private SQLiteDatabase db;
	
	public KalmanInfoHelper(Context context) {
		super(context);
		openDb();
	}
    ...
    public long insert(GPSInfo gpsInfo) {   
        ...
        return db.insert(DbHelper.TRACKER_KALMAN_DB_TABLE, null, values);
    }
    ...
    public List<GPSInfo> getGPSPoint() {
		...
		String sql = "SELECT * FROM " + DbHelper.TRACKER_KALMAN_DB_TABLE;
    ...
    public GPSInfo getPointInfo(int index) {
    	String sql = "SELECT * FROM " + DbHelper.TRACKER_KALMAN_DB_TABLE + " WHERE _id = " + index;
		GPSInfo gpsInfo = new GPSInfo();
		Cursor cursor = db.rawQuery(sql, null);
	...
	public void cleanOldRecords() {
        db.delete(DbHelper.TRACKER_KALMAN_DB_TABLE, null, null);
    }
